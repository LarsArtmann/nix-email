...
| R3     | Smarthost relay spike: ~~Mailpit as mock submission upstream in VM, verify `queue.*` keys on 0.15.5~~ mechanism VERIFIED locally (`c927922`; loopback guard blocks the VM path)                                                                              | Research      | Critical     | 90m     | -                     | ~~Round-trip two-node VM test in TODO_LIST~~ done - stalwart-relay-e2e shipped in v0.2.0 (598db0f); the real Resend path waits for D2 (TODO_LIST, BLOCKED)            |
...
| ~~H2~~     | ~~CI workflow (adapted nix-check.yml: flake check + input hygiene + fail-closed)~~ done — CI shipped in v0.2.0 - fail-closed, action-SHA-validated, strict lockstep guard (598db0f) | ~~Hardening~~ | ~~High~~ | ~~60m~~ | ~~-~~ | ~~Every push gated like sibling repos~~ |
| ~~H3~~     | ~~Formatter + pre-commit: treefmt/alejandra/statix/deadnix, run over repo~~ done — formatter shipped: alejandra flake output + dprint, CI-enforced (v0.2.0); pre-commit hooks deliberately replaced by the CI gate | ~~Hardening~~ | ~~Medium~~ | ~~45m~~ | ~~-~~ | ~~Repo matches LarsArtmann conventions~~ |
...
| ~~H5~~     | ~~Module defaults from spike findings: `services.mail-server.relay.*` option + DKIM defaults + ledger updates~~ done — relay option + DKIM subtest shipped in v0.2.0 (598db0f) | ~~Hardening~~ | ~~Critical~~ | ~~90m~~ | ~~R2,R3~~ | ~~Module becomes deployable, not just evaluable~~ |
| H6     | d2 architecture diagram in README + ~~AGENTS.md rules (assertions-from-transcripts, no-pipes-on-gates)~~ (`95d5dcb` + 2026-09-15)                                                                                                                            | Hardening     | Medium       | 30m     | -                     | ~~Diagram task open in TODO_LIST~~ done - d2 in README + rendered SVGs (2026-09-15)                                                         |
| H7     | LICENSE + ~~.gitignore polish~~ (`21cab2c`) + ~~`/tmp/swtest` cleanup~~ (verified gone)                                                                                                                                                                      | Hardening     | Medium       | 30m     | D3                    | ~~LICENSE blocked on the license choice (ROADMAP Q3)~~ resolved 2026-09-15: MIT confirmed and shipped (v0.2.0)                                     |
| ~~I1~~     | ~~SystemNix: add `nix-email` flake input + consumer wrapper skeleton (ports.nix, sops, onFailure, harden)~~ done — SystemNix consumer wrapper shipped (input pinned to tag v0.2.0, relay assertions restored) | ~~Integration~~ | ~~Critical~~ | ~~90m~~ | ~~D3~~ | ~~The "pluggable into SystemNix" promise becomes real~~ |
...
| Tier                                 | State                                                                                                      |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| Decisions                            | D3 fully decided (public + MIT, 2026-09-15); D1, D2 open (ROADMAP "Open questions")                        |
| Research                             | R1-R5 done; R3 round-trip VM test DONE (stalwart-relay-e2e, v0.2.0); R6 blocked on D1 (TODO_LIST)          |
| Hardening                            | ALL done (H1-H7): CI, formatter, relay/DKIM options, diagrams, LICENSE shipped across v0.1.0/v0.2.0        |
| Integration                          | I1 DONE (consumer wrapper, pins v0.2.0); I2, I3 gated on D1/VPS (ROADMAP)                                  |
| VPS / Terraform / Migration / Future | Gated on D1/D2 - ROADMAP themes (untouched, correctly)                                                     |

Updated 2026-09-16 (docs-health pass): every ungated tier shipped; the
remaining open rows (D1/D2, R6, I2/I3, V/T/M/F tiers) are tracked in
ROADMAP.md and TODO_LIST.md. Micro-task statuses follow their section-2
parents. Archived.
