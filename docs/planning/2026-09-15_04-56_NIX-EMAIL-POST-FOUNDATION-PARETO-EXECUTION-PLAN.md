...
| C1  | SystemNix: `nix-email` flake input + consumer wrapper skeleton (`modules/nixos/services/nix-email.nix`, enable gates) + ports.nix registration                     | Integration | Critical | 90m    | -                 | THE 51% move: the pluggable promise becomes real                       |
| C2  | SystemNix: sops templates (fallback-admin secret, relay secret placeholder) + `services.stalwart.credentials` wiring + onFailure→Discord                           | Integration | High     | 60m    | C1                | Secrets never in the store; failures page Lars                         |
...
| C4  | SystemNix: eval + VM test importing the upstream module (mock-sops pattern)                                                                                        | Integration | High     | 60m    | C1                | Consumer wiring stays green like the upstream gate                     |
| Q1  | CI: GitHub Action `nix flake check`, fail-closed, asserts checks actually ran (Files>0-class guard)                                                                | Quality     | High     | 60m    | -                 | Every push gated like sibling repos; no false greens                   |
| Q2  | Nix formatter (alejandra/nixfmt per fleet convention) + one mechanical pass over modules/tests/flake                                                               | Quality     | Medium   | 45m    | -                 | Repo matches LarsArtmann conventions; dprint covers the rest already   |
| Q3  | LICENSE file + GitHub license metadata (per D3)                                                                                                                    | Quality     | Medium   | 10m    | D3                | Public repo becomes legally usable by others                           |
| Q4  | Root-cause the `mkDefault`-list-dropped-on-firewall-allowedTCPPorts behavior (nixpkgs 26.11) or find/file upstream issue                                           | Quality     | Medium   | 45m    | -                 | Only the symptom is banked; a pin-bump trap for every future module    |
| Q5  | TODO_LIST hygiene: add `verified <date>` stamps; reconcile session-born items                                                                                      | Quality     | Low      | 30m    | -                 | Verification staleness becomes auditable                               |
| Q6  | Docs polish bundle: CONTRIBUTING (ledger-bullet rules) + architecture diagram in README                                                                            | Quality     | Medium   | 60m    | -                 | Contributors can extend the ledger; the system explains itself         |
| Q7  | aarch64 posture: run the VM test under qemu once, or document x86_64-only loudly                                                                                   | Quality     | Medium   | 30m    | -                 | Closes the "never executed on ARM" honesty gap                         |
| Q8  | Repo polish bundle: GitHub topics + git-town.toml + Renovate (nixpkgs-input pairing note)                                                                          | Quality     | Low      | 60m    | -                 | Discovery + workflow parity with siblings + dependency hygiene         |
| Q9  | Threat-model doc: loopback-guard boundary, admin exposure policy, secret inventory                                                                                 | Quality     | Medium   | 60m    | -                 | Security posture written down before the VPS exists                    |
| T1  | Two-node VM test (stalwart + Mailpit node): submission → `queue.route` sink → assert arrival (beats the loopback-refusal guard)                                    | Test        | High     | 90m    | -                 | The smarthost path gets CI coverage; closes R3's open half             |
| T2  | DKIM signing VM test: declarative `signature.<id>` + test key; assert `DKIM-Signature` header; optional POST /api/dkim variant                                     | Test        | High     | 60m    | -                 | Signing is verified, not just key-name-documented                      |
| T3  | Restart-persistence VM test: needle survives `systemctl restart`; anonymous admin API stays 401                                                                    | Test        | Medium   | 30m    | -                 | RocksDB state + auth persistence proven                                |
| T4  | Metrics VM assertion: `metrics.prometheus.enable` → `/metrics/prometheus` 200 (401 with auth)                                                                      | Test        | Medium   | 30m    | -                 | Closes the "endpoint never probed" gap before V4 alerting builds on it |
| T5  | Backup/restore VM drill: `--export`, wipe store, `--import`, assert needle survival                                                                                | Test        | High     | 60m    | -                 | The backup claim becomes tested fact before the VPS depends on it      |
| T6  | parsedmarc E2E VM test: dovecot node + seeded DMARC report mailbox → JSON/CSV output asserted                                                                      | Test        | Medium   | 60m    | -                 | The parser runs in CI; dmarc-monitor leaves eval-only status           |
| T7  | E2E subtest bundle: quota (1KB bounce), alias/catch-all, Junk-folder delivery (ONE VM file)                                                                        | Test        | Medium   | 60m    | -                 | Mailbox semantics covered without multiplying CI time                  |
| T8  | Regression VM test: `is_local_domain` negative-cache ordering (the poisoning fix guard)                                                                            | Test        | Medium   | 30m    | -                 | The nastiest bug of the foundation cannot silently return              |
| T9  | Details-level journal assertion with curated benign-filter (resolver/pyzor/ASN lines)                                                                              | Test        | Low      | 30m    | -                 | The no-crash gate sees event details, not just names                   |
| T10 | Guard bundle: dmarc-eval `versionOlder` pin-move guard + parsedmarc systemd hardening mkDefaults                                                                   | Test        | Low      | 40m    | -                 | Survives nixpkgs bumps; hardened unit defaults                         |
| M1  | `services.mail-server.relay = { address, port, username, secretFile; }` option → verified `queue.route`+`queue.strategy.route` TOML + eval assertion + test + docs | Module      | Critical | 90m    | -                 | The ledger recipe becomes product; deployable outbound in one option   |
| M2  | `certificate` tier option (`self-signed \| acme \| manual`) with mutual-exclusion assertions (nms x509 pattern)                                                    | Module      | High     | 60m    | -                 | Cert management is one enum, not TOML archaeology                      |
| M3  | Small options bundle: `metrics.enable` wrapper + httpBind non-loopback warning assertion                                                                           | Module      | Medium   | 45m    | -                 | Metrics in one flag; the reverse-proxy doctrine is enforced            |
| M4  | `cache.ttl.negative` wrapper option (dev/test hosts want it low)                                                                                                   | Module      | Medium   | 30m    | -                 | Kills the 1h negative-cache trap where it bites                        |
