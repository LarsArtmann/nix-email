...
| - | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ~~1~~ | ~~**parsedmarc-e2e green**~~ done — parsedmarc-e2e green since the 2026-09-15 17-05 session (two nodes since the evening session) | ~~Eval passes (dovecot 2.4 version pins)~~ | ~~Runtime still red for a ONE-LINE reason now known: `wait_for_unit("dovecot2.service")` → must be `dovecot.service` (a/12). Then re-run + full `nix flake check` gate (cached-verdict trap: input changed, will re-eval)~~ |
| ~~2~~ | ~~**SystemNix C1-C4 consumer wiring**~~ done — C1 completed 2026-09-15: input + lock + sops file + contract test, all green; pin advanced to tag v0.2.0 | ~~Wrapper module written (a/10); lock-rev parity verified (`eaad089` both repos); deployment-state documented (plumbing-only, enabled nowhere)~~ | ~~flake.nix input NOT added (interrupted at exactly this edit twice); sops placeholder secrets file NOT created; `tests/test-nix-email.nix` NOT written; SystemNix eval gate NOT run; SystemNix git repo BROKEN (see d/1)~~ |
| ~~3~~ | ~~**Docs sync (docs-health debt)**~~ done — docs synced in the 17-05 session (TODO_LIST/CHANGELOG/FEATURES/ROADMAP/AGENTS); CHANGELOG duplicate-Changed merged 2026-09-16 | ~~—~~ | ~~TODO_LIST.md still shows ~19 rows as TODO that are DONE in code+CHANGELOG; CHANGELOG.md structurally mangled (old `[Unreleased]` section orphaned below the new one — duplicate Added/Changed/Fixed tails); ROADMAP license question #3 needs the MIT-executed annotation; TODO rows lack `verified <date>` stamps (TODO row)~~ |
| ~~4~~ | ~~VM debug loop~~ **Won't implement — /tmp artifacts ephemeral by design; the debug pattern is preserved in-tree (tests/fixtures/debug-template.py).** | ~~Driver realized + debug script ran to completion twice~~ | ~~Third run's full output unanalyzed beyond the dovecot finding; `/tmp` artifacts fragile across interruptions (lost once already)~~ |

## c) NOT STARTED (deliberate — interrupted, or sequenced after the gate)

1. ~~**E2E subtests: quota, alias/catch-all, Junk-delivery** (TODO row; keys verified a/9, incl. the `spam-filter.*` namespace correction — the TODO's assumed mechanism ("spam.header") does not exist in 0.15.5)~~ done (quota + alias + catch-all + GTUBE subtests shipped in v0.2.0; Junk-delivery NOT-DO (sieve wall, ROADMAP Q6))
2. ~~**Negative-cache regression VM test** (plan: poison → provision → short `directoryCacheTtlNegative` → assert local delivery after TTL expiry; doubles as coverage for the new option)~~ done (negative-cache regression pair shipped in v0.2.0)
3. ~~**d2 architecture diagram in README** (hosts, flows, decisions — TODO row)~~ done (d2 + SVGs shipped 2026-09-15)
4. ~~**TODO_LIST verified-date stamps** (TODO row, folds into b/3)~~ done (verified stamps shipped (rows carry verified dates))
5. ~~**SystemNix sops secrets** (`platforms/nixos/secrets/nix-email.yaml`: `dmarc-imap-password`, `stalwart-fallback-admin`, `stalwart-relay-password` placeholders, age-encrypted per `.sops.yaml` creation rule)~~ done (platforms/nixos/secrets/nix-email.yaml created (sops, 3 placeholder keys))
6. ~~**SystemNix test** (mock-sops pattern, co-import integration.nix per the mkIf/options-guard trap note; eval-contract level — upstream already VM-tests product behavior)~~ done (tests/test-nix-email.nix shipped (13 assertions green; relay assertions restored at pin v0.2.0))
7. ~~**Renovate pairing note** (nix-email input must move with SystemNix's nixpkgs — noted in the planned input comment, config not touched)~~ done (renovate.json pairing note + PIN comments shipped)
8. ~~Upstream-issue candidates verified-but-not-filed: nixpkgs parsedmarc localMail dovecot 2.4 gap (eval-breaking), podman/udp-over-tcp `lib.optional` firewall defs (mkDefault-killer), dovecot2.service rename (breaks wait_for_unit callers). verify-before-filing + github-voice apply before any filing.~~ done (two filed 2026-09-15 (NixOS/nixpkgs#563651, #563652) and linked from the ledger; podman lib.optional + dovecot rename documented in ledger entries (no separate filings))
...
1. ~~Authorize + execute SystemNix object repair (delete 0-byte object, `git fetch`, verify `git log`/`status` — a/d/1, ~5 min)~~ done (recovery executed; SystemNix fsck-clean (17-05 a/14))
2. ~~Authorize + execute nix-email history recovery (fetch, `git switch -c recover origin/master`, commit working-tree delta, then fast-forward master without reset-class commands — a/d/2, ~10 min)~~ done (recovery executed (fetch + working-tree commit path); history intact)
3. ~~Re-run `git fsck` both repos post-repair; confirm zero missing objects~~ done (fsck clean both repos (17-05 session))

**Gate back to green (nix-email):**
4. ~~Apply the one-line dovecot rename fix (`wait_for_unit("dovecot.service")`, a/12) — also re-check `postfix.service`/unit names in that script for other 26.11 renames while there~~ done (dovecot rename fix shipped; ledger bullet covers it)
5. ~~Re-run parsedmarc-e2e alone → green~~ done (green since 17-05 session)
6. ~~Full `nix flake check` (no pipes, background, read raw output) → all 4 checks green~~ done (full gate green (all 4 checks))
7. ~~Commit gate-green state explicitly (per-task message, not daemon heuristic)~~ done (release commit 598db0f is explicit (per-task message))

**TODO_LIST execution (remaining rows, ranked):**
8. ~~Quota subtest (per-principal `quota` column, a/9 — unit needs empirical verification, likely bytes; assert via rejection or GETQUOTA)~~ done (quota subtest shipped (bytes semantics source-verified; assert IMAP absence))
9. ~~Alias/catch-all subtest (multi-email principal + `@example.test` catch-all; generalize the imap probe for per-account logins)~~ done (alias/catch-all subtests shipped; generalized per-account probes shipped)
10. ~~Junk-delivery subtest (`spam-filter.rule.<id>` header rule + `spam-filter.score.spam`; keys binary-verified a/9; rule field names need one empirical probe — docs fetch 404'd)~~ **Won't implement — GTUBE subtest shipped instead; Junk filing proven impossible on 0.15.5 (sieve wall) - ROADMAP Q6 carries the revised options.**
11. ~~Negative-cache regression subtest (short `directoryCacheTtlNegative`, poison→provision→wait-TTL→assert local)~~ done (regression pair shipped (directoryCacheTtlNegative = 5 in test))
12. ~~Ledger entries for quota/alias/junk/spam-filter keys once observed in VM (fact+method+date)~~ done (ledger entries shipped (quota, catch-all, spam defaults, spam-filter namespace))
13. ~~d2 architecture diagram in README (hosts, flows, decisions — keep it small, dprint-stable)~~ done (d2 + SVGs shipped)
14. ~~TODO_LIST sync: delete ~19 done rows, add `verified 2026-09-15` stamps to the rest (schema per docs-health e/5)~~ done (TODO_LIST synced with verified stamps)
15. ~~CHANGELOG restructure (merge orphaned old `[Unreleased]` Added/Changed/Fixed into the current sections; keep Keep-a-Changelog shape)~~ done (CHANGELOG restructured (17-05 session); duplicate Changed sections merged 2026-09-16)
16. ~~ROADMAP: annotate open question #3 as executed-MIT (2026-09-15, mandate + standing recommendation, revisit-before-first-external-contribution)~~ done (ROADMAP Q3 annotated RESOLVED (MIT))
17. ~~FEATURES.md: add hardening/LICENSE/CI/topics rows to inventory~~ done (FEATURES rows shipped (hardening/LICENSE/CI/topics))
18. ~~Renovate config: add nix-email pairing note if not adequately covered by the input comment (check existing renovate.json)~~ done (renovate.json pairing note shipped)
19. ~~aarch64 posture row: README now documents x86_64-only loudly — verify the row's "or document" arm is satisfied and delete the row~~ done (aarch64 row deleted; posture decided (documented-manual))

**SystemNix C1-C4 completion:**
20. ~~Add the nix-email flake input (exact edit prepared twice, both interrupted — comment text in session transcript)~~ done (flake input added; pin at tag v0.2.0)
21. ~~`nix flake lock --update-input nix-email` (or `nix flake update nix-email`) + parity check (rev == eaad089-pin doctrine)~~ done (lock updated; parity held (eaad089 both repos at C1 time))
22. ~~Create `platforms/nixos/secrets/nix-email.yaml` (sops -e to evo-x2 age key per `.sops.yaml`; placeholder values documented as rotate-at-go-live)~~ done (sops placeholder file created)
23. ~~Write `tests/test-nix-email.nix` (mock-sops + integration.nix co-import; assert: `_secret` path wiring, credentials → LoadCredential macro, registry entry fan-out, unit onFailure)~~ done (contract test shipped + registered)
24. ~~Register test in `tests/default.nix`~~ done (registered in tests/default.nix)
25. SystemNix eval gate (their `nix flake check` equivalent / pre-commit suite) → green
26. ~~SystemNix explicit commit (per-task message; daemon races — check status first)~~ done (committed (daemon + explicit commits; pin-advance landed))
27. ~~Note in nix-email README "SystemNix integration" section: consumer wrapper landed, shape link~~ done (README SystemNix integration section documents the landed wrapper)

**Cross-repo hygiene:**
28. ~~Push nix-email recover branch / master (needs authorization — push was authorized last session for that push only)~~ done (pushed (1f8bb52..b80137f and later))
29. SystemNix push (if the daemon's 4f0b90 push was partial — verify remote integrity post-repair)
30. ~~Consider filing the 3 upstream issues (c/8) — verify-before-filing gate, then github-voice drafting~~ done (two filed + linked from ledger; third (Restart policy) routed to TODO_LIST)
31. ~~Git-town/config alignment check if sibling-repo workflow expects it (git-town.toml exists nix-email side)~~ **Won't implement — git-town.toml in use; no alignment issue reported.**
...
3. ~~**LICENSE confirmation:** I executed MIT under the mandate (standing recommendation, sibling convention). Confirm, or name your preferred alternative while it is still trivially changeable (no external contributors yet).~~ done (MIT CONFIRMED by the user 2026-09-15; LICENSE stands)

## Resolution addendum (2026-09-16, docs-health pass)

All recovery, gate, docs, and C1-C4 items resolved inline. Untouched: f/25
(SystemNix full gate - pre-existing deadnix failures, routed to the SystemNix
CI-debt TODO row), f/29 (SystemNix push - user-gated TODO row), f/32-34
(D1/D2-gated - TODO_LIST/ROADMAP), g/2 (D1 - ROADMAP open question 1).
Archived.
