...
| Restart-persistence VM test: deliver, `systemctl restart stalwart`, message still in INBOX                                         | 🔴 `TODO`     | Med    | 30min  | RocksDB state survival never asserted (`tests/stalwart-e2e.nix` has no restart subtest) |
