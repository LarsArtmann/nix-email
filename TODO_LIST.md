...
| Root-cause (or find the upstream issue for) the `mkDefault`-list-dropped-on-`networking.firewall.allowedTCPPorts` behavior in nixpkgs 26.11 base modules - only the symptom is banked | 🔴 `TODO` | Low    | 45min  | `AGENTS.md` convention + `modules/mail-server.nix:126-129`; probe series documented in `docs/status/2026-09-15_04-42` d/7 |
| Add `verified <date>` stamps to TODO_LIST rows so verification staleness is auditable at the next docs-health pass | 🔴 `TODO` | Low    | 30min  | `docs/status/2026-09-15_04-42` e/5                                               |
