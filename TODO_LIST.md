...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SystemNix: advance the `nix-email` pin past the relay-landing rev, then restore the relay-credential assertions in `tests/test-nix-email.nix` and delete the wrapper's option-existence guard (both mark the spot) | 🔴 `TODO` (unblocked: nix-email pushed 2026-09-15) | High | 30min | verified 2026-09-15: wrapper guard + PIN NOTE in SystemNix `modules/nixos/services/nix-email.nix` / `tests/test-nix-email.nix`; upstream `relay` option now on origin/master |
...
| Cut `v0.1.0`: move `[Unreleased]` into a dated section, tag, and open the GitHub release (CI's first real run rides the same push) | 🔴 `TODO` (unblocked: pushed 2026-09-15) | High | 20min | verified 2026-09-15: CHANGELOG has `[Unreleased]` + `[0.1.0]` but no tag/release exists; push authorized and executed |
...
| Task                                                                                                                                                                          | Status                              | Impact | Effort | Evidence                                                                                                                                                              |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LICENSE: pick and confirm the license (MIT was shipped under the earlier blanket mandate; the user REJECTED confirming MIT on 2026-09-15 - naming the wanted license flips this row, the change is one file) | 🔵 `BLOCKED` (user: which license?) | Low | 5min | verified 2026-09-15: `LICENSE` in tree says MIT; user answer "no" to MIT confirmation - the target license is unknown |
...
| stalwart-e2e: strengthen the over-quota retry evidence (assert a SECOND `Message rescheduled for delivery` after a sleep) so "retried forever" is not proven by a single line | 🔴 `TODO`                            | Low    | 10min  | verified 2026-09-15: the subtest asserts one reschedule line + IMAP absence; "forever" rests on the ledger claim, not the transcript        |
